...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--
       ...
       -->
  <xsl:template match="para[@condition = 'adagio.flv.player']">
    <xsl:if test="(phrase[@condition = 'file'] != '') and @id and
      ...
      <xsl:variable name="shockwave.pluginspace">
        <xsl:choose>
          ...
          select="normalize-space($adagio.flv.player.pluginspace)"/></xsl:otherwise>
        </xsl:choose>
      </xsl:variable>

      <div class="adagio_flv_video">
      ...
      <script type="text/javascript">
        <xsl:attribute name="src"><xsl:value-of select="normalize-space($adagio.flv.player.js.file)"/></xsl:attribute></script>
      <script type="text/javascript">
        var s1 = new SWFObject("<xsl:value-of select="normalize-space($adagio.flv.player.swf.file)"/>",
                               "mediaplayer",
                               "<xsl:value-of select="$shockwave.width"/>",
                               "<xsl:value-of select="$shockwave.height"/>",
                               "<xsl:value-of select="$adagio.flv.player.minimum.version"/>");
