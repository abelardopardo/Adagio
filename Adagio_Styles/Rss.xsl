...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    <rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
      <channel>
        <xsl:if test="$adagio.rss.self.atom.link != ''">
          <atom:link rel="self" type="application/rss+xml">
            <xsl:attribute name="href"><xsl:value-of
                select="$adagio.rss.self.atom.link" />
            </xsl:attribute>
          </atom:link>
        </xsl:if>
        <xsl:if test="$adagio.rss.time.to.live != ''">
          <ttl><xsl:value-of select="$adagio.rss.time.to.live"/></ttl>
        </xsl:if>

        <xsl:if test="$adagio.rss.title != ''">
          <title><xsl:value-of select="$adagio.rss.title"/></title>
        </xsl:if>
        <xsl:if test="($adagio.rss.title != '') and ($adagio.rss.image.url != '')
                      and ($adagio.rss.main.site.url != '')">
          <image>
            <title><xsl:value-of select="$adagio.rss.title"/></title>
            <url><xsl:value-of select="$adagio.rss.image.url"/></url>
            <link><xsl:value-of select="$adagio.rss.main.site.url"/></link>
            <xsl:if test="$adagio.rss.image.desc != ''">
              <description><xsl:text
              disable-output-escaping="yes">&lt;![CDATA[</xsl:text><xsl:value-of
              select="$adagio.rss.image.desc"/><xsl:text
              disable-output-escaping="yes">]]&gt;</xsl:text></description>
            </xsl:if>
            <width><xsl:value-of select="$adagio.rss.image.width"/></width>
            <height><xsl:value-of select="$adagio.rss.image.height"/></height>
          </image>
        </xsl:if>

        <xsl:if test="$adagio.rss.description != ''">
          <description><xsl:text
          disable-output-escaping="yes">&lt;![CDATA[</xsl:text><xsl:value-of
          select="$adagio.rss.description"/><xsl:text
          disable-output-escaping="yes">]]&gt;</xsl:text></description>
        </xsl:if>
        <xsl:if test="$adagio.rss.main.site.url != ''">
          <link><xsl:value-of select="$adagio.rss.main.site.url"/></link>
        </xsl:if>
        <xsl:if test="$adagio.rss.language != ''">
          <language><xsl:value-of select="$adagio.rss.language"/></language>
        </xsl:if>
        <xsl:if test="$adagio.rss.copyright != ''">
          <copyright><xsl:value-of select="$adagio.rss.copyright"/></copyright>
        </xsl:if>

        <!-- Crucial elements that need to be produced every time with
             the correct format -->
        <lastBuildDate><xsl:value-of select="$adagio.rss.date.rfc822"/></lastBuildDate>
        <pubDate><xsl:value-of select="$adagio.rss.date.rfc822"/></pubDate>
        <docs>http://blogs.law.harvard.edu/tech/rss</docs>
        <xsl:if test="$adagio.rss.author.email != ''">
          <webMaster><xsl:value-of select="$adagio.rss.author.email"/></webMaster>
        </xsl:if>
        <!-- ITunes Stuff -->

        <!-- Author name in text -->
        <xsl:if test="$adagio.rss.author.name != ''">
          <itunes:author><xsl:value-of select="$adagio.rss.author.name"/></itunes:author>
        </xsl:if>
        <!-- One line for channel subtitle, similar to description above -->
        <xsl:if test="$adagio.rss.subtitle != ''">
          <itunes:subtitle><xsl:value-of
          select="$adagio.rss.subtitle"/></itunes:subtitle>
        </xsl:if>

        <!-- One paragraph explaining in more detaile what the channel offers -->
        <xsl:if test="$adagio.rss.summary">
          <itunes:summary><xsl:value-of select="normalize-space($adagio.rss.summary)"/></itunes:summary>
        </xsl:if>

        <!-- Redundant information about the author -->
        <xsl:if test="($adagio.rss.author.name != '') and ($adagio.rss.author.email != '')">
          <itunes:owner>
            <itunes:name><xsl:value-of select="$adagio.rss.author.name"/></itunes:name>
            <!-- Author Email -->
            <itunes:email><xsl:value-of select="$adagio.rss.author.email"/></itunes:email>
          </itunes:owner>
        </xsl:if>

        <!-- If the content in the item is explicit -->
        <xsl:if test="$adagio.rss.explicit != ''">
          <itunes:explicit><xsl:value-of
          select="$adagio.rss.explicit"/></itunes:explicit>
        </xsl:if>

        <!-- An image that is displayed somewhere -->
        <xsl:if test="$adagio.rss.image.url != ''">
          <itunes:image>
            <xsl:attribute name="href"><xsl:value-of
            select="$adagio.rss.image.url"/></xsl:attribute>
          </itunes:image>
        </xsl:if>

        <!-- Category and subcategory information -->
        <xsl:if test="$adagio.rss.category != ''">
          <itunes:category>
            <xsl:attribute name="text"><xsl:value-of select="$adagio.rss.category"/></xsl:attribute>
            <xsl:if test="$adagio.rss.subcategory != ''">
              <itunes:category>
                <xsl:attribute name="text"><xsl:value-of
                select="$adagio.rss.subcategory"/></xsl:attribute>
              </itunes:category>
            </xsl:if>
          </itunes:category>
        </xsl:if>
	...
	<xsl:apply-templates
	  select="exsl:node-set($rssitems)[position()>last()-$adagio.rss.numitems.max]">
  ...
  <xsl:template match="sectioninfo[contains(@condition, 'rss.info')]|
    ...
    <xsl:variable name="date.now">
      <xsl:choose>
        <xsl:when test="($adagio.rss.force.date) and ($adagio.rss.force.date != '')">
          <!--          <xsl:value-of select="$adagio.rss.force.date" /> -->
          <xsl:value-of select="$adagio.rss.date.rfc822" />
        </xsl:when>
    ...
    <!-- If date check is off or itemdate is later than right now,
         then dump the item -->
    <xsl:if test="($adagio.rss.check.pubdate = '0') or
      ...
      <item>
        ...
        <link>
	  <xsl:choose>
	    <xsl:when test="$adagio.rss.autolink='true'">
	      <!-- Link auto-numbering -->
	      <xsl:value-of
		select="$adagio.rss.autolink.urlprefix" />#item<xsl:number
		...
		<xsl:if test="ancestor::*/ulink[@condition = 'itemlinkbase']">
		  <xsl:value-of select="$adagio.rss.item.url.prefix"/><xsl:value-of
        ...
        <guid>
          <xsl:choose>
            <xsl:when test="$adagio.rss.autoguid='true'">
              <!-- GUID auto-numbering -->
	      <xsl:value-of
		select="$adagio.rss.autoguid.guidprefix" />#item<xsl:number
		value="$numitems - position() + 1" format="1"/>
            </xsl:when>
            <xsl:when test="modespec/olink[@condition = 'guid']">
              ...
              select="$adagio.rss.item.url.prefix"/><xsl:value-of
              select="ancestor::*/ulink[@condition =
              'itemlinkbase']/@url"/>/</xsl:if><xsl:value-of
        ...
        <pubDate>
          <xsl:choose>
            ...
            <xsl:otherwise><xsl:value-of select="$adagio.rss.date.rfc822"/></xsl:otherwise>
          </xsl:choose>
        </pubDate>
